...
var SwipeView = (function (window, document) {
	var dummyStyle = document.createElement('div').style,
		...
		SwipeView = function (el, options) {
			...
			addEventListener(window,	   resizeEvent, this, false);
			addEventListener(this.wrapper, startEvent, this, false);
			addEventListener(this.wrapper, moveEvent, this, false);
			addEventListener(this.wrapper, endEvent, this, false);
			addEventListener(this.slider,  transitionEndEvent, this, false);
...
/*			if (!hasTouch) {
	...
	SwipeView.prototype = {
		...
		onFlip: function (fn) {
			addEventListener(this.wrapper, 'swipeview-flip', fn, false);
			this.customEvents.push(['flip', fn]);
		},

		onAfterFlip: function (fn) {
			addEventListener(this.wrapper, 'swipeview-afterflip', fn, false);
			this.customEvents.push(['afterflip', fn]);
		},
		
		onMoveOut: function (fn) {
			addEventListener(this.wrapper, 'swipeview-moveout', fn, false);
			this.customEvents.push(['moveout', fn]);
		},

		onMoveIn: function (fn) {
			addEventListener(this.wrapper, 'swipeview-movein', fn, false);
			this.customEvents.push(['movein', fn]);
		},
		
		onTouchStart: function (fn) {
			addEventListener(this.wrapper, 'swipeview-touchstart', fn, false);
...
/*			if (newX > 0 || newX < this.maxX) {
	...
	function addEventListener(where, event, target, flag) {
		if (typeof where.addEventListener != 'undefined')
			where.addEventListener(event, target, flag);
		if (typeof where.attachEvent != 'undefined')
			where.attachEvent(event, target);
	}
